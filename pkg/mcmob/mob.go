...
// drawPivotedBoxUV draws a textured box rotated around a pivot point (useful for legs/arms)
func drawPivotedBoxUV(x, y, z, w, h, d float32, u, v, tw, th, td float32, imgW, imgH float32, tint rl.Color, pivotY, rotX float32) {
	rl.PushMatrix()
	rl.Translatef(x, pivotY, z)
	rl.Rotatef(rotX*(180.0/math.Pi), 1.0, 0.0, 0.0)
	rl.Translatef(-x, -pivotY, -z)
	
	drawBoxUV(x, y, z, w, h, d, u, v, tw, th, td, imgW, imgH, tint)
...
func (m *Mob) renderZombie(tint rl.Color, tex rl.Texture2D) {
	if tex.ID > 0 {
		...
		// Legs (4x12x4)
		legSwing := float32(math.Sin(float64(m.WalkBobbing))) * 0.8
		drawPivotedBoxUV(-0.13, 0.38, 0.0, 0.22, 0.72, 0.22, 0, 16, 4, 12, 4, 64, 64, tint, 0.74, legSwing)
		drawPivotedBoxUV(0.13, 0.38, 0.0, 0.22, 0.72, 0.22, 0, 16, 4, 12, 4, 64, 64, tint, 0.74, -legSwing)

		// Arms (4x12x4)
		drawPivotedBoxUV(-0.34, 1.25, 0.0, 0.20, 0.72, 0.20, 40, 16, 4, 12, 4, 64, 64, tint, 1.6, -legSwing)
		drawPivotedBoxUV(0.34, 1.25, 0.0, 0.20, 0.72, 0.20, 40, 16, 4, 12, 4, 64, 64, tint, 1.6, legSwing)
...
func (m *Mob) renderCreeper(tint rl.Color, tex rl.Texture2D) {
	...
	if tex.ID > 0 {
		...
		legSwing := float32(math.Sin(float64(m.WalkBobbing))) * 0.8
		drawPivotedBoxUV(-0.15, 0.22, 0.16, 0.18, 0.44, 0.18, 0, 16, 4, 6, 4, 64, 32, tint, 0.44, legSwing)
		drawPivotedBoxUV(0.15, 0.22, 0.16, 0.18, 0.44, 0.18, 0, 16, 4, 6, 4, 64, 32, tint, 0.44, -legSwing)
		drawPivotedBoxUV(-0.15, 0.22, -0.16, 0.18, 0.44, 0.18, 0, 16, 4, 6, 4, 64, 32, tint, 0.44, -legSwing)
		drawPivotedBoxUV(0.15, 0.22, -0.16, 0.18, 0.44, 0.18, 0, 16, 4, 6, 4, 64, 32, tint, 0.44, legSwing)
		rl.SetTexture(0)
...
func (m *Mob) renderPig(tint rl.Color, tex rl.Texture2D) {
	if tex.ID > 0 {
		...
		legSwing := float32(math.Sin(float64(m.WalkBobbing))) * 0.8
		drawPivotedBoxUV(-0.18, 0.18, 0.24, 0.18, 0.36, 0.18, 0, 16, 4, 6, 4, 64, 32, tint, 0.36, legSwing)
		drawPivotedBoxUV(0.18, 0.18, 0.24, 0.18, 0.36, 0.18, 0, 16, 4, 6, 4, 64, 32, tint, 0.36, -legSwing)
		drawPivotedBoxUV(-0.18, 0.18, -0.24, 0.18, 0.36, 0.18, 0, 16, 4, 6, 4, 64, 32, tint, 0.36, -legSwing)
		drawPivotedBoxUV(0.18, 0.18, -0.24, 0.18, 0.36, 0.18, 0, 16, 4, 6, 4, 64, 32, tint, 0.36, legSwing)
		rl.SetTexture(0)
...
func (m *Mob) renderCow(tint rl.Color, tex rl.Texture2D) {
	if tex.ID > 0 {
		...
		legSwing := float32(math.Sin(float64(m.WalkBobbing))) * 0.8
		drawPivotedBoxUV(-0.22, 0.24, 0.32, 0.18, 0.48, 0.18, 0, 16, 4, 12, 4, 64, 32, tint, 0.48, legSwing)
		drawPivotedBoxUV(0.22, 0.24, 0.32, 0.18, 0.48, 0.18, 0, 16, 4, 12, 4, 64, 32, tint, 0.48, -legSwing)
		drawPivotedBoxUV(-0.22, 0.24, -0.32, 0.18, 0.48, 0.18, 0, 16, 4, 12, 4, 64, 32, tint, 0.48, -legSwing)
		drawPivotedBoxUV(0.22, 0.24, -0.32, 0.18, 0.48, 0.18, 0, 16, 4, 12, 4, 64, 32, tint, 0.48, legSwing)
		rl.SetTexture(0)
...
func (m *Mob) renderSheep(tint rl.Color, tex rl.Texture2D) {
	if tex.ID > 0 {
		...
		legSwing := float32(math.Sin(float64(m.WalkBobbing))) * 0.8
		drawPivotedBoxUV(-0.20, 0.20, 0.28, 0.16, 0.40, 0.16, 0, 16, 4, 6, 4, 64, 32, tint, 0.40, legSwing)
		drawPivotedBoxUV(0.20, 0.20, 0.28, 0.16, 0.40, 0.16, 0, 16, 4, 6, 4, 64, 32, tint, 0.40, -legSwing)
		drawPivotedBoxUV(-0.20, 0.20, -0.28, 0.16, 0.40, 0.16, 0, 16, 4, 6, 4, 64, 32, tint, 0.40, -legSwing)
		drawPivotedBoxUV(0.20, 0.20, -0.28, 0.16, 0.40, 0.16, 0, 16, 4, 6, 4, 64, 32, tint, 0.40, legSwing)
		rl.SetTexture(0)
